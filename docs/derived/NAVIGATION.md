# Corpus Navigation (derived)

- Class: DERIVED, NON-AUTHORITATIVE -- informative documentation only
  (Baseline L-2). It defines nothing; on any conflict the repository
  source documents win and this file is regenerated.
- Ownership: P13 documentation pipeline
  (`docs/program/DOC_DERIVATION_CONVENTIONS.md`); never hand-edited.
- Generated by: `tools/docs/derive_navigation.py`.
- Regenerate from the repository root: `python tools/docs/derive_navigation.py`
  (or `python tools/docs/build_docs.py` to rebuild all derived docs).
- Output: `docs/derived/NAVIGATION.md`.
- Sources:
  - `docs/constitution/*.md`
  - `docs/architecture/*.md`
  - `docs/decisions/ADR-*.md`
  - `docs/rfc/RFC-*.md`
- Limitations: mechanical extraction only -- no summaries, no
  interpretation, no status or disposition synthesis, no behavior
  claims.
- Titles are verbatim first headings; files without a heading fall back to the filename.
- Entry points are linked by path only; their content is not derived from.

## Constitution (`docs/constitution/`)

- [Article 1 — Mission](../constitution/01-mission.md) -- `docs/constitution/01-mission.md`
- [Article 2 — Vision](../constitution/02-vision.md) -- `docs/constitution/02-vision.md`
- [Article 3 — Core Values](../constitution/03-core-values.md) -- `docs/constitution/03-core-values.md`
- [Article 4 — Engineering Principles](../constitution/04-engineering-principles.md) -- `docs/constitution/04-engineering-principles.md`
- [Article 5 — Architecture Principles](../constitution/05-architecture-principles.md) -- `docs/constitution/05-architecture-principles.md`
- [Article 6 — Governance](../constitution/06-governance.md) -- `docs/constitution/06-governance.md`
- [Article 7 — Decision Making](../constitution/07-decision-making.md) -- `docs/constitution/07-decision-making.md`
- [Article 8 — Compatibility Policy](../constitution/08-compatibility-policy.md) -- `docs/constitution/08-compatibility-policy.md`
- [Article 9 — Release Policy](../constitution/09-release-policy.md) -- `docs/constitution/09-release-policy.md`
- [Article 10 — Long-term Goals](../constitution/10-long-term-goals.md) -- `docs/constitution/10-long-term-goals.md`
- [Frontier Creative OS Constitution](../constitution/README.md) -- `docs/constitution/README.md`

## Architecture (`docs/architecture/`)

- [Architecture Documents](../architecture/README.md) -- `docs/architecture/README.md`
- [Frontier Creative OS — Architecture Baseline](../architecture/baseline.md) -- `docs/architecture/baseline.md`
- [Frontier Creative OS — Master System Blueprint](../architecture/blueprint.md) -- `docs/architecture/blueprint.md`
- [Frontier Creative OS — Universal Engineering Doctrine](../architecture/doctrine.md) -- `docs/architecture/doctrine.md`
- [FSL Stage 2 Behavior & Architecture Guide](../architecture/stage2_behavior.md) -- `docs/architecture/stage2_behavior.md`

## Decision records (ADRs) (`docs/decisions/`)

- [ADR-0001 — Ratification of Constitution v1](../decisions/ADR-0001.md) -- `docs/decisions/ADR-0001.md`
- [ADR-0002 — Adoption of the Architecture Document Set](../decisions/ADR-0002.md) -- `docs/decisions/ADR-0002.md`
- [ADR-0003 — Ratification of the Engineering Prompt Class](../decisions/ADR-0003.md) -- `docs/decisions/ADR-0003.md`
- [ADR-0004 — Disposition of the RFC Decision Set (RFC-0001–RFC-0017)](../decisions/ADR-0004.md) -- `docs/decisions/ADR-0004.md`
- [ADR-0005 — Specification Corpus Home: `/specs`](../decisions/ADR-0005.md) -- `docs/decisions/ADR-0005.md`
- [ADR-0006 — Language Identity: Definition Core with Staged Evolution (I-D)](../decisions/ADR-0006.md) -- `docs/decisions/ADR-0006.md`
- [ADR-0007 — FSL First-Class Consumers and Success Criteria (RFC-0001 Q1/Q2)](../decisions/ADR-0007.md) -- `docs/decisions/ADR-0007.md`
- [ADR-0008 — Namespace Registry Authority, Identifier Grandfathering, and Token Discipline (RFC-0002)](../decisions/ADR-0008.md) -- `docs/decisions/ADR-0008.md`
- [ADR-0009 — S04 Container Specification Ratification (P03.1)](../decisions/ADR-0009.md) -- `docs/decisions/ADR-0009.md`
- [ADR-0010 — Concrete M-B Clause-Index Schema (P03.4)](../decisions/ADR-0010.md) -- `docs/decisions/ADR-0010.md`
- [ADR-0011 — S04 Stage 0 Minimal Contract Clause Set, Sidecar Instantiation, and Drift-Checker Authorization (P03.3 + ADR-0010 Follow-ups 1–2)](../decisions/ADR-0011.md) -- `docs/decisions/ADR-0011.md`
- [ADR-0012 — S04 Stage 1 Entry with the W-B Interchange-Kernel Scope (RFC-0019 Acceptance)](../decisions/ADR-0012.md) -- `docs/decisions/ADR-0012.md`
- [ADR-0013 — RFC-0020 Acceptance: Stage 1 W-B Clause-Set Ratification with JSON as the Concrete Interchange Form (F1/F2 Resolution)](../decisions/ADR-0013.md) -- `docs/decisions/ADR-0013.md`
- [ADR-0014 — Ratification of the S05 Compiler Subsystem Boundary Contract Specification](../decisions/ADR-0014.md) -- `docs/decisions/ADR-0014.md`
- [ADR-0015 — Ratification of the S13 Verification & Conformance Subsystem Specification](../decisions/ADR-0015.md) -- `docs/decisions/ADR-0015.md`
- [ADR-0016 — RFC-0023 Acceptance: S04 Stage 2 Capability Transition & Scope Ratification (D1: Enter Stage 2, D2: Scope Package S2-B, D3: Static Evaluation Only, D4: Additive Extension to fsl/1.0)](../decisions/ADR-0016.md) -- `docs/decisions/ADR-0016.md`

## RFCs (dispositions recorded in ADR-0004) (`docs/rfc/`)

- [RFC-0001 — Language Identity and Design Philosophy](../rfc/RFC-0001.md) -- `docs/rfc/RFC-0001.md`
- [RFC-0002 — Artifact Identifier and Namespace Scheme](../rfc/RFC-0002.md) -- `docs/rfc/RFC-0002.md`
- [RFC-0003 — Specification Corpus Location and Repository Taxonomy](../rfc/RFC-0003.md) -- `docs/rfc/RFC-0003.md`
- [RFC-0004 — Conformance Levels and Third-Party Certification](../rfc/RFC-0004.md) -- `docs/rfc/RFC-0004.md`
- [RFC-0005 — Compatibility and Stability Policy](../rfc/RFC-0005.md) -- `docs/rfc/RFC-0005.md`
- [RFC-0006 — Reference Implementation Policy](../rfc/RFC-0006.md) -- `docs/rfc/RFC-0006.md`
- [RFC-0007 — Package Distribution Model](../rfc/RFC-0007.md) -- `docs/rfc/RFC-0007.md`
- [RFC-0008 — Machine-Readable Specification Format](../rfc/RFC-0008.md) -- `docs/rfc/RFC-0008.md`
- [RFC-0009 — Language Versioning Scheme](../rfc/RFC-0009.md) -- `docs/rfc/RFC-0009.md`
- [RFC-0010 — Multi-Maintainer Governance Procedure](../rfc/RFC-0010.md) -- `docs/rfc/RFC-0010.md`
- [RFC-0011 — CI Platform and Gate Encoding](../rfc/RFC-0011.md) -- `docs/rfc/RFC-0011.md`
- [RFC-0012 — Runtime Scope](../rfc/RFC-0012.md) -- `docs/rfc/RFC-0012.md`
- [RFC-0013 — Candidate Subsystem: Planner](../rfc/RFC-0013.md) -- `docs/rfc/RFC-0013.md`
- [RFC-0014 — Candidate Subsystem: Memory](../rfc/RFC-0014.md) -- `docs/rfc/RFC-0014.md`
- [RFC-0015 — Candidate Subsystem: Context Engine](../rfc/RFC-0015.md) -- `docs/rfc/RFC-0015.md`
- [RFC-0016 — Candidate Subsystem: Agent Framework](../rfc/RFC-0016.md) -- `docs/rfc/RFC-0016.md`
- [RFC-0017 — Candidate Subsystem: Product UI](../rfc/RFC-0017.md) -- `docs/rfc/RFC-0017.md`
- [RFC-0018 — S04 Stage 0 Minimal Contract Clause Set (Amendment Proposal)](../rfc/RFC-0018.md) -- `docs/rfc/RFC-0018.md`
- [RFC-0019 — S04 Stage 1 Language-Content Scope (Minimum P04 Unlock)](../rfc/RFC-0019.md) -- `docs/rfc/RFC-0019.md`
- [RFC-0020 — S04 Stage 1 W-B Interchange-Kernel Clause Set (Amendment Proposal)](../rfc/RFC-0020.md) -- `docs/rfc/RFC-0020.md`
- [RFC-0021 — S05 Compiler Subsystem Boundary Contract Specification](../rfc/RFC-0021.md) -- `docs/rfc/RFC-0021.md`
- [RFC-0022 — S13 Verification & Conformance Subsystem Specification](../rfc/RFC-0022.md) -- `docs/rfc/RFC-0022.md`
- [RFC-0023 — S04 Stage 2 Capability-Stage Transition & Scope Framing](../rfc/RFC-0023.md) -- `docs/rfc/RFC-0023.md`

## Repository entry points (path links only)

- [`README.md`](../../README.md)
- [`CONTRIBUTING.md`](../../CONTRIBUTING.md)
- [`docs/rfc/README.md`](../rfc/README.md)
- [`docs/prompts/README.md`](../prompts/README.md)
- [`docs/program/PROGRAM_INDEX.md`](../program/PROGRAM_INDEX.md)
- [`specs/README.md`](../../specs/README.md)
